...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const menu = document.querySelector('.type-filter-menu');
    ...
    options.forEach(option => {
        option.addEventListener('click', function() {
            ...
            const selectedOptions = Array.from(document.querySelectorAll('.type-option.selected'));
            
            // Cập nhật badge
            updateFilterBadge(selectedOptions.length);
            
            // Dispatch custom event
            const event = new CustomEvent('typeFilterChanged', {
                detail: { 
                    selectedTypes: selectedOptions.map(opt => opt.querySelector('.type-label').textContent.trim())
                }
            });
